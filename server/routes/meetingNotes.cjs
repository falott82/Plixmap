...
const registerMeetingNoteRoutes = (app, deps) => {
  ...
  // Shared OpenAI notes transform (translate / correct). Both the meeting-notes and the
  // client-notes endpoints validate the body, enforce the per-user daily token budget, call
  // OpenAI and audit the outcome identically — only resource resolution, the audit event
  // prefix, the audited scope id and the extra audit details differ. The caller resolves the
  // client + access control, then delegates here.
  const runNotesAiTransform = async (req, res, { client, usageScopeId, eventPrefix, extraDetails = {} }) => {
    ...
    if (!apiKey) {
      const clientLabel = String(client?.shortName || client?.name || usageScopeId || 'cliente');
    ...
    const usageKey = `${usageDay}|${String(usageScopeId || '')}|${String(req.userId || '')}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        writeAuditLog(db, {
          ...
          scopeType: 'client',
          scopeId: String(usageScopeId || ''),
          ...requestMeta(req),
          details: { ...extraDetails, mode, status: response.status, detail: String(detail || '').slice(0, 300) }
        });
      ...
      writeAuditLog(db, {
        ...
        scopeId: String(usageScopeId || ''),
        ...requestMeta(req),
        details: { ...extraDetails, mode, detail: String(detail || '').slice(0, 300) }
      });
  ...
  app.get('/api/meetings/:id/notes-export', requireAuth, (req, res) => {
    ...
    const esc = (value) => {
      let raw = String(value == null ? '' : value).replace(/\r?\n/g, ' ');
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      await runNotesAiTransform(req, res, {
        client,
        usageScopeId: String(booking.clientId || ''),
        eventPrefix: 'meeting_notes_ai_transform',
        extraDetails: { meetingId }
      });
    }
  );
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      await runNotesAiTransform(req, res, {
        client,
        usageScopeId: clientId,
        eventPrefix: 'client_notes_ai_transform',
        extraDetails: {}
      });
    }
  );

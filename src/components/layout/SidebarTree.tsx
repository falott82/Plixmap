...
import { useSecurityCardVisibility } from './useSecurityCardVisibility';
...
const SidebarTree = () => {
  ...
  const { isSecurityCardVisibleForPlan, toggleSecurityCardVisibilityForPlan } = useSecurityCardVisibility({
    planLayerIdsByPlan,
    visibleLayerIdsByPlan,
    hiddenLayersByPlan,
    setVisibleLayerIds,
  ...
  const openClientMeetingDuplicateModal = (booking: MeetingBooking, options?: { preferredDay?: string }) => {
    ...
    const { roomOptions, selectedRoomId } = buildDuplicateRoomOptions(
      siteEntry,
      booking,
      t({ it: 'Sala meeting', en: 'Meeting room' })
    );

...
import { hmToMinutes, monthAnchorFromIso, shiftMonthAnchor, timeHmFromTs, todayIso } from './SidebarTree.helpers';

type Translate = (msg: { it: string; en: string }) => string;

export type SidebarClientMeetingsDuplicateModalProps = {
  clientMeetingsDuplicateModal: any;
  clientMeetingsDuplicateFocusRef: MutableRefObject<any>;
  clientMeetingsDuplicateRoomPickerOpen: boolean;
  clientMeetingsDuplicateRoomPickerRef: MutableRefObject<any>;
  saveClientMeetingDuplicates: (...args: any[]) => any;
  setClientMeetingsDuplicateModal: Dispatch<SetStateAction<any>>;
  setClientMeetingsDuplicateRoomPickerOpen: Dispatch<SetStateAction<any>>;
  t: Translate;
};

/**
 * Client-meeting duplication modal (target day/time, room picker, multi-day
 * scheduling). Extracted from SidebarTree.tsx.
 */
export const SidebarClientMeetingsDuplicateModal = ({
  clientMeetingsDuplicateModal,
  clientMeetingsDuplicateFocusRef,
  clientMeetingsDuplicateRoomPickerOpen,
  clientMeetingsDuplicateRoomPickerRef,
  saveClientMeetingDuplicates,
  setClientMeetingsDuplicateModal,
  setClientMeetingsDuplicateRoomPickerOpen,
  t
}: SidebarClientMeetingsDuplicateModalProps) => {
  return (
              ...
              >
                <Dialog.Panel className="w-full max-w-[760px] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl">
                  {(() => {
                    ...
                    const selectedRoomOption = dup.roomOptions.find((entry: any) => String(entry.roomId) === String(dup.selectedRoomId));
                            ...
                            <div className="mt-3 space-y-3">
                              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                ...
                                <div ref={clientMeetingsDuplicateRoomPickerRef} className="relative mt-2">
                                  <button
                                    type="button"
                                    onClick={() => setClientMeetingsDuplicateRoomPickerOpen((prev: any) => !prev)}
                                  ...
                                  {clientMeetingsDuplicateRoomPickerOpen ? (
                                    <div className="absolute z-[220] mt-1 max-h-72 w-full overflow-auto rounded-xl border border-slate-200 bg-white p-1 shadow-xl">
                                      <button
                                        type="button"
                                        onClick={() => {
                                          setClientMeetingsDuplicateModal((prev: any) => (prev ? { ...prev, roomMode: 'any', error: null } : prev));
                                      ...
                                      {dup.roomOptions.map((option: any) => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                              setClientMeetingsDuplicateModal((prev: any) =>
                                      ...
                                      onClick={() =>
                                        setClientMeetingsDuplicateModal((prev: any) => {
                                          ...
                                          return {
                                            ...
                                            error: null,
                                            selectedDays: has
                                              ? prev.selectedDays.filter((entry: any) => entry !== cell.iso)
